...
public class EngineeringServiceImpl implements EngineeringService {
	...
	// static final String ENDPOINT_PAGAMENTI = "http://payweb.ve.eng.it/pa/services/PagamentiTelematiciDovutiPagati"; // TEST ENGINEERING
		...
		for (Pagamento p : pagamenti) {
			if (StringUtils.isNotBlank(p.getIdSession())) {
				...
				if (fault != null && fault.value != null) {
					...
					p.setDataEsitoSingoloPagamento(item.getDataEsitoSingoloPagamento().toGregorianCalendar().getTime());
					...
					log.setDataEsitoSingoloPagamento(item.getDataEsitoSingoloPagamento().toGregorianCalendar().getTime());
