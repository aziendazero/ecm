...
public interface FieldIntegrazioneAccreditamentoRepository extends CrudRepository<FieldIntegrazioneAccreditamento, Long> {
	...
	@Query(value = "SELECT fi.* FROM ecmdb.field_integrazione_accreditamento fi "
			+ "		INNER JOIN ecmdb.field_integrazione_history_container fihc ON fihc.id = fi.field_integrazione_history_container_id"
			+ "		WHERE fihc.accreditamento_id = :accreditamentoId AND fihc.stato = :stato AND fihc.work_flow_process_instance_id = :workFlowProcessInstanceId AND fi.fittizio = TRUE", nativeQuery = true)
	public Set<FieldIntegrazioneAccreditamento> findAllFittiziByContainer(@Param("accreditamentoId") Long accreditamentoId, @Param("stato") String stato, @Param("workFlowProcessInstanceId") Long workFlowProcessInstanceId);
}
