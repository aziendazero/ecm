	...
	@Bean(name={"javers"})
	  public Javers javers(ConnectionProvider connectionProvider, PlatformTransactionManager transactionManager)
	  {
	    ...
	    return TransactionalJaversBuilder.javers()
	      ...
	      .withListCompareAlgorithm(ListCompareAlgorithm.valueOf(this.javersProperties
	      .getAlgorithm().toUpperCase()))
	      .withMappingStyle(MappingStyle.valueOf(this.javersProperties
	      .getMappingStyle().toUpperCase()))
	      .withNewObjectsSnapshot(this.javersProperties
	      .isNewObjectSnapshot())
	      .withPrettyPrint(this.javersProperties
	      .isPrettyPrint())
	      .withTypeSafeValues(this.javersProperties
	      .isTypeSafeValues())
	      .withPackagesToScan(this.javersProperties
	      .getPackagesToScan())
	      .build();
	  }
}
